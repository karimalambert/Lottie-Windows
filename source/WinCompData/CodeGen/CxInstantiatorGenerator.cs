...
public:
    virtual Microsoft::UI::Xaml::Controls::IAnimatedVisual^ TryCreateAnimatedVisual(
        ...
        /// <inheritdoc/>
        protected override string GenerateCompositeEffectFactory(CodeBuilder builder, CompositionEffectBrush obj)
        {
            builder.WriteLine($"ComPtr<CompositeEffect> compositeEffect(new CompositeEffect());");
            builder.WriteLine($"compositeEffect->SetMode({CanvasCompositeMode(obj.Effect.Mode)});");
            foreach (var source in obj.Effect.Sources)
            {
                builder.OpenScope();
                builder.WriteLine($"auto sourceParameter = ref new CompositionEffectSourceParameter({String(source)});");
